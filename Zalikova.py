...
def create_signals(k_lines):
    ...
    for i in range(len(k_lines)):
        ...
        if (k_lines.iloc[i]['ema'] > k_lines.iloc[i]['vwma'] and
            k_lines.iloc[i]['sma'] > k_lines.iloc[i]['ema'] and
            k_lines.iloc[i]['rsi'] < 30 and
            k_lines.iloc[i]['adx'] > 20):
            signal = 'buy'
        elif (k_lines.iloc[i]['ema'] < k_lines.iloc[i]['vwma'] and
              k_lines.iloc[i]['sma'] < k_lines.iloc[i]['ema'] and
              k_lines.iloc[i]['rsi'] > 70 and
              k_lines.iloc[i]['adx'] > 20):
            signal = 'sell'
        else:
            continue
        ...
        if signal == "buy":
            stop_loss_price = round(current_price * 0.9925, 2)  # 0.75% stop loss
            take_profit_price = round(current_price * 1.0215, 2)  # 2.15% take profit
        elif signal == "sell":
            stop_loss_price = round(current_price * 1.0075, 2)  # 0.75% stop loss
            take_profit_price = round(current_price * 0.9785, 2)  # 2.15% take profit
